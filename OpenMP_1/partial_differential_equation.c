...
int main(int argc, const char * argv[]){
  ...
  //Timesteps cannot be parallelized, they depend on the previous one
  //Parts in RuKu3 can be parallelized
  //makes sense, if nx is large (>500)
...
void RuKu3(double t,double dt,int nx, double y0[nx], double x[nx], int nghost) {
  ...
  //each block can be parallelized, but they depend on the previous one
  //
...
void der2(int nx, double fx[nx], double x[nx], double der2f[nx], int nghost) {
  ...
  case 1:
    ...
    #pragma omp parallel for shared(der2f, dx, nx)
    for (int i=0;i<nx;i++) {
